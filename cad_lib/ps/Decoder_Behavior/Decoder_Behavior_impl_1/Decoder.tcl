if { ![project_exists {Decoder}] } {
     project_new {Decoder}
} else {
     project_open {Decoder}
}
set_project_settings -cmp {Decoder}
if { ![project_settings_exist -sim Decoder] } {
        set_project_settings -sim Decoder
}
set_global_assignment -section_id {Decoder} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {Decoder.edf}
...
if { [file exists {Decoder_altera.prop}] } {
  source Decoder_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {Decoder} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {Decoder_pnr_constraints.sdc}
