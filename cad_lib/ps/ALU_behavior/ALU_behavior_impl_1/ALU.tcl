...
set_project_settings -cmp {ALU}
if { ![project_settings_exist -sim ALU] } {
        set_project_settings -sim ALU
}
set_global_assignment -section_id {ALU} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {ALU.edf}
...
if { [file exists {ALU_altera.prop}] } {
  source ALU_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {ALU} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {ALU_pnr_constraints.sdc}
